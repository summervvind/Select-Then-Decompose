    ...
    async def evaluate_problem(self, problem: dict, method) -> Tuple[str, str, str, str, str, float, float, float]:
        ...
        try:
            if self.params.method == "select_then_decompose":
                output, final_method, cost = await self._generate_output(method, inputs)
            else:
                output, cost = await self._generate_output(method, inputs)
            ...
            return input_text, context_str, final_method, output, format_output, expected_output, score, total_cost, total_calls

        except Exception as e:
            print(f"Maximum retries reached. Skipping this sample. Error: {e}")
            return input_text, context_str, str(e), str(e), str(e), expected_output, 0.0, 0.0, 0.0

    def get_result_columns(self) -> List[str]:
        return ["question", "context", "final_method", "prediction", "format_output", "expected_output", "score", "cost", "total_calls"]
